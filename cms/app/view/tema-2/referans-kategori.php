...
<section class="jumbotron text-center">
    <div class="container">
        <h1><?= $curr_category['category_name'] ?></h1>
        <div class="breadcrumb-custom">
            <a href="<?= site_url() ?>">Anasayfa</a> /
            <a href="<?= site_url('referanslar') ?>">Referanslar</a> /
            <a href="<?= site_url('referanslar/' . $curr_category['category_url']) ?>"
               class="active"><?= $curr_category['category_name'] ?></a>
        </div>
    </div>
</section>
            ...
            <div class="col-md-12 text-center">
                <nav class="nav nav-pills nav-fill mb-4">
                    <a href="<?= site_url('referanslar') ?>"
                       class="nav-item nav-link<?= !route(1) ? ' active' : null ?>">Tümü</a>
                    <?php foreach ($categories as $category): ?>
                        <a class="nav-item nav-link<?= route(1) == $category['category_url'] ? ' active' : null ?>"
                           href="<?= site_url('referanslar/' . $category['category_url']) ?>"><?= $category['category_name'] ?></a>
                    <?php endforeach; ?>
                </nav>
